...
import org.leibnizcenter.cfg.earleyparser.EarleyFunctions;
...
//        this.options = options;
        ...
        if (listener != null) listener.parserSeeded(new EdgeEvent(index, seedEdge));
    }


    private void fireEdgePredicted(int index, Edge edge) {
        if (listener != null) {
            listener.edgePredicted(new EdgeEvent(index, edge));
        }
    }
    ...
    public Parse<T> parseNextToken() throws PepException {
        EarleyFunctions.predict(grammar, chart, index, predictPreterm, listener); // make predictions at this index

        Token<T> token = tokenIterator.next(); // get next token
        parse.addToken(token); // add to tokens in parse

        EarleyFunctions.scan(grammar, chart, index, token, predictPreterm, listener); // scan and increment index
        index++;
        EarleyFunctions.complete(chart, index, listener); // complete for next index

        if (!tokenIterator.hasNext()) {
            // finish filling chart by predicting for final index
            EarleyFunctions.predict(grammar, chart, index, predictPreterm, listener);
